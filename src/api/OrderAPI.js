...
export async function fetchOrders(accountID) {
    ...
    const token = getAuthToken();
    try {
        const response = await fetch(`https://diamondstore.lemonhill-6b585cc3.eastasia.azurecontainerapps.io/api/orders/account/${accountID}`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
...
export const handleVnpayReturn = async (params) => {
    try {
        const token = getAuthToken();
        const response = await axios.get(`https://diamondstore.lemonhill-6b585cc3.eastasia.azurecontainerapps.io/api/payment/vnpay_return`, {
            params: params,
            headers: {
                Authorization: `Bearer ${token}`
            }
...
export const getPromotion = async (promotionCode) => {
    try {
        const response = await axios.get(`https://diamondstore.lemonhill-6b585cc3.eastasia.azurecontainerapps.io/api/promotion/{promotionCode}?${promotionCode}`, {
...
export async function fetchOrderDetail(orderID) {
    const token = getAuthToken();
    try {
      const response = await axios.get(`https://diamondstore.lemonhill-6b585cc3.eastasia.azurecontainerapps.io/api/orderDetail/getOrderDetail?orderID=${orderID}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
...
export const getTotalOrderMoney = async (orderId) => {
    try {
      const token = getAuthToken();
      const response = await axios.get(`https://diamondstore.lemonhill-6b585cc3.eastasia.azurecontainerapps.io/api/orders/totalOrder/${orderId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
